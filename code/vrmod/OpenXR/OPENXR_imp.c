...
static qboolean oxr_CreateXrInstance( void ) {
	...
	strncpy(createInfo.applicationInfo.applicationName, "Quake 3 Arena", XR_MAX_APPLICATION_NAME_SIZE);
	strncpy(createInfo.applicationInfo.engineName, "Q3e Vulkan OpenXR", XR_MAX_ENGINE_NAME_SIZE);
...
qboolean OpenXR_get_swapchain_format(VkSurfaceFormatKHR surface_format)
{
	...
	for (uint32_t i = 0; i < swapchainFormatCount; i++) {
		ri.Printf(PRINT_ALL, "[%i] swapchain formats : %li", i, swapchainFormats[i]);
	}
